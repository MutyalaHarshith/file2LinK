...
# Running bot
mhbot = Client('File-Sharing', api_id=APP_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
# Notify about bot start
with mhbot:
    mhbot_username = xbot.get_me().username  # Better call it global once due to telegram flood id
    print("Bot started!")
    mhbot.send_message(int(OWNER_ID), "Bot started!")


# Start & Get file
@mhbot.on_message(filters.command('start') & filters.private)
...
# Help msg
@mhbot.on_message(filters.command('help') & filters.private)
async def _help(bot, update):
    await update.reply_text("Supported file types:\n\n- Video\n- Audio\n- Photo\n- Document\n- Sticker\n- GIF\n- Voice note\n- Video note\n\n If bot didn't respond, contact @MutyalaHarshith", True)
    ...
    await update.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton('Sharing Link',
                                  url=f'https://t.me/{mhbot_username}?start={unique_idx.lower()}-{str(msg_id)}')]
        ])
    )
...
# Store media_group
media_group_id = 0
@mhbot.on_message(filters.media & filters.private & filters.media_group)
